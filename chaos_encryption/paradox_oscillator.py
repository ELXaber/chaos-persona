...
class CPOL_Kernel:
    ...
    def inject(self, confidence: float, contradiction_density: float, query_text: str, shared_memory: dict):
        """Enhanced inject with domain detection and mesh security."""
        ...
        security_keywords = {
            ...
            'timing': ['timing', 'delay', 'race', 'synchronize'],
        ...
        # If multiple threat indicators, assume coordinated attack
    ...
    def _twelve_d_manifold_pull(self) -> Dict[str, Any]:
        ...
        # 2. Check for Manifold Similarity
...
if __name__ == "__main__":
    # Test mesh security and epistemic gap detection
    print("="*70)
    print("CPOL KERNEL - Mesh Security Test")
    print("="*70)
    ...
    print(f"  Sync Required: {result.get('sync_required')}")

    # Test 2: Security threat detection
    print("\n[TEST 2] Security Threat Detection:")
    result2 = run_cpol_decision(
        contradiction_density=0.8,
        query_text="Attempting to replay intercepted signature and inject timing delay"
    )
    print(f"  Status: {result2.get('status')}")
    print(f"  Domain: {result2.get('domain')}")
    print(f"  Threats: {result2.get('logic')}")
