...
# =============================================================================
# CPOL INTENT MODES - Intent-Aware Safety (replaces heavy mesh section)
# =============================================================================
CPOL_INTENT_MODES = {
    # Creative / generative – monitor only to avoid invalid outputs
...
def determine_cpol_mode(intent: str = "", query_text: str = "") -> str:
    """Determine mode based on intent or query keywords."""
    text_lower = (intent + " " + query_text).lower()
    for key, mode in CPOL_INTENT_MODES.items():
        if key in text_lower:
            return mode
    return "full"  # Default: maximum safety
...
class CPOL_Kernel:
    ...
    def inject(self, confidence: float, contradiction_density: float, query_text: str, shared_memory: Optional[dict] = None):
        ...
        # Intent-aware mode + smart cycle override
        intent = shared_memory.get('intent', '')
        self.cpol_mode = determine_cpol_mode(intent, query_text)

        # Smart cycle limit
        distress = shared_memory.get('distress_density', 0.0)
        high_risk = any(m in query_text.lower() for m in ["security", "threat", "attack", "jump", "overdose", "suicide", "cliff"])
        if (self.cpol_mode == "full" or distress > 0.75 or high_risk or 
            any(k in query_text.lower() for k in ["paradox", "prove", "validate", "solve"])):
            self.limit_run = 50
        else:
            self.limit_run = 11  # Efficient default
        ...
        # --- STEP 2: SAFETY OVERRIDE (GENERALIZED - Non-Security Risks) ---
    ...
    def run_cpol_decision(self, 
                          prompt_complexity: str = "high", 
                          contradiction_density: float = None,
                          query_text: str = "",
                          shared_memory: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Run CPOL decision as an instance method."""

        # Auto-detect or use provided contradiction density
        ...
        # Use provided shared_memory or create default
        if shared_memory is None:
            shared_memory = {'distress_density': 0.0}
        ...
        print(f"[CPOL] Domain: {self.current_domain} | Density: {density:.2f} | Evidence: {self.evidence_score:.2f}")
        result = self.oscillate()
