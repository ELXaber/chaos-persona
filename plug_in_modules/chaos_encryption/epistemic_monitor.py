...
def calculate_dynamic_jitter_threshold(state: Dict[str, Any]) -> float:
    """
    Ties the Manifold's internal sync threshold to the system's distress density.
    Formula: Base (0.001) modulated by Distress (0.0 to 1.0).
    """
    distress = state.get('distress_density', 0.0)

    # At 0.0 distress (Perfect Calm): Threshold is 0.005 (More elastic)
    # At 1.0 distress (Chaos Lock): Threshold is 0.0001 (High precision/Rigid)
    # This creates an inverse relationship: higher distress = tighter tolerances.
    dynamic_threshold = 0.005 * (1.0 - (distress * 0.98))

    return max(0.0001, dynamic_threshold)
