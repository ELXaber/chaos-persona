...
def log_discovery(
    ...
    if cpol_trace and 'signature' in cpol_trace:
        manifold_sig = cpol_trace['signature']

    # 2. Build the entry with the Signature Anchor
    entry = {
    ...
    discovery_id = hashlib.sha256(entry_json.encode()).hexdigest()
